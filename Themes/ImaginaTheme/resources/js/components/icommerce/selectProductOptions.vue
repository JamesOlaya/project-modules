      ...
      <div class="content-option" v-if="option.productOptionValues &&  option.productOptionValues.length">
        ...
        <div v-if="option.type == 'select'">
          <select class="form-control" v-model="section[option.id].singleOption"
            ...
            <option v-for="(selectOption,key) in getOptionsSelect(option.productOptionValues)" :key="key"
                    :value="selectOption.id">{{selectOption.label}}
            </option>
          </select>
        </div>
        ...
        <div v-if="option.type == 'radio'" class="ml-3">
          <div v-for="(value, key) in option.productOptionValues" :key="key"
               :class="`text-left ${value.optionValueEntity.options.type != 1 ? 'd-inline-block' : ''}`">
            <!--Color box-->
            <div v-if="value.optionValueEntity.options.type == 3"
                 ...
                 v-on:click="setOption(option, value.optionValueId)"
                 :style="`background-color : ${value.optionValueEntity.options.color}`"></div>
            <!--Image box-->
            <div v-else-if="value.optionValueEntity.options.type == 2"
                 :class="`box-image ${section[option.id].singleOption == value.optionValueId ? 'box-image-active' : ''}`"
                 v-on:click="setOption(option, value.optionValueId)"
                 :style="`background-image : url(${value.optionValueEntity.mainImage.path})`"></div>
            <!--Input radio-->
            <div v-else>
      ...
      getOptionsSelect(options) {
        if (options && options.length) {
          let response = options.map(item => {
            return {label: item.optionValue, id: item.optionValueId}
          })
          //Add price to label
